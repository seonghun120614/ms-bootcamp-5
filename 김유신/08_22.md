...
---
- 두 방식의 단점
    - Math.random() : 내부적으로 단일 Random 객체를 공유해서 여러 스레드가 동시에 쓰면 락(lock) 때문에 성능 저하
...
- 단, **매개변수의 개수·타입·순서**가 달라야 함
- 반환형만 다르다고 오버로딩이 되지는 않음

### 특징
1. 메서드 이름은 같아야 한다.
2. 매개변수의 **개수/타입/순서**가 달라야 함
3. 반환형은 오버로딩 성립에 영향을 주지 않음
4. 컴파일 시점에 어떤 메서드가 실행될지 결정 (**정적 바인딩**)
